...
object Blur {
    ...
    fun blurScreen(activity: Activity, view: View) {
        ...
        Completable.create {
            try {
                val bitmap = Blur.drawViewToBitmap(width, height, view, Color.parseColor("#fff6f6f6"))
                val blurredBitmap = Blur.fastBlur(activity.baseContext, bitmap, 25f)
                bitmap.recycle()
                Blur.saveBitmapToFile(activity, blurredBitmap)
            ...
            Timber.e("It took ${System.currentTimeMillis() - start}ms to blur")
        }
    ...
    fun loadLatest(context: Activity): BitmapDrawable? {
        return try {
            val bitmap = loadBitmapFromFile(context, "blur")
    ...
    private fun fastBlur(context: Context, sentBitmap: Bitmap, radius: Float): Bitmap {
        ...
        val paint = Paint()
        val filter = LightingColorFilter(-0xbbbbbc, 0x66333333)
